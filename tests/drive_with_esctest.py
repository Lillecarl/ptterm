...
#: run.
#:
#: An exclusion is not a recorded failure. A failure says "ptterm
#: differs from xterm here, and an entry in DEVIATIONS.md says why". An
...
NOT_OURS = (
    (
        ...
        "ptterm draws inside somebody else's window, so it has none to "
        "move, to iconify, or to report the position of. DEVIATIONS.md "
        "entry 15.",
    ),
)
...
def keep(directory: Path, failed, log: str) -> None:
    ...
    (directory / "failures.txt").write_text(
        ...
        "#\n"
        "# pymux/tests/esctest-failures.txt is the same list for ptterm in a\n"
        "# pymux pane. A name there and not here is what the pane adds.\n"
        "#\n"
