...
#
# Two checks run it. `ptterm-esctest` puts it on a pty of its own, so what it
# judges is ptterm alone. `pymux-esctest` runs it in a pane, so what it judges
# is ptterm and everything pymux puts around it. pymux takes this package from
# `ptterm.esctest2`, which is why it lives here.
#
